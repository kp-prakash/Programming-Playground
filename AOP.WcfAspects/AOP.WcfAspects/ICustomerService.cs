using System.Collections.Generic;
using System.ServiceModel;
using AOP.WcfAspects.DataContracts;

namespace AOP.WcfAspects
{
    // NOTE: You can use the "Rename" command on the "Refactor" menu to change the interface name "ICustomerService" in both code and config file together.
    [ServiceContract]
    public interface ICustomerService
    {
        [OperationContract]
        IEnumerable<ActiveCustomerForListing> FetchActiveCustomersForListing();

        [OperationContract]
        IEnumerable<CustomerForListing> FetchCustomersForListing();

        [OperationContract]
        void UpdateCustomerUsing(CustomerForUpdating customerToUpdate);
    }
}