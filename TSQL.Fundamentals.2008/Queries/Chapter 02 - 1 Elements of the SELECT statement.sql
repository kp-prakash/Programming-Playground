...
/*OFFSET-FETCH Filter - SQL Server 2012
Standard SQL defines a TOP-like filter called OFFSET-FETCH
that does support skipping capabilities, and this makes it 
very useful for ad-hoc paging purposes.

SQL Server 2012 introduces support for the OFFSET-FETCH filter.
The OFFSET-FETCH filter in SQL Server 2012 is considered part 
of the ORDER BY clause, which normally serves a presentation 
ordering purpose. By using the OFFSET clause, you can indicate
how many rows to skip, and by using the FETCH clause, you can 
indicate how many rows to filter after the skipped rows.

* A query with OFFSET FETCH must have an ORDER BY clause.
* Fetch clause is not supported without OFFSET clause. Instead use OFFSET 0 ROWS.
* OFFSET without 
*/
...
--SQL Server allows us to use ROW and ROWS interchangeably.
SELECT orderid, orderdate, custid, empid
FROM Sales.Orders
ORDER BY orderdate, orderid
...
FROM Sales.Orders
ORDER BY orderdate, orderid
OFFSET 800 ROWS; --Returns 30 rows of total 830 as 800 is skipped!

--FIRST and NEXT are interchangeable
SELECT orderid, orderdate, custid, empid
FROM Sales.Orders
ORDER BY orderdate, orderid
OFFSET 50 ROWS FETCH FIRST 25 ROWS ONLY; --Returns 30 rows of total 830 as 800 is skipped!
