<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:msxsl="urn:schemas-microsoft-com:xslt" exclude-result-prefixes="msxsl">
    ...
    <xsl:variable name="displaymethods" xml:space="preserve"><xsl:choose>
                                                            <xsl:when test="string(//./params[@name = 'displaymethods']/@value) = '1'">True</xsl:when>
                                                            <xsl:otherwise>False</xsl:otherwise></xsl:choose></xsl:variable>
    ...
    <xsl:variable name="ccColumnCount" xml:space="preserve"><xsl:choose>
                                                            ...
                                                            <xsl:otherwise>0</xsl:otherwise></xsl:choose></xsl:variable>
    <!--Used by the new cc grouping report-->
    <xsl:variable name="ccMethodDefinedMin" xml:space="preserve"><xsl:call-template name="min"><xsl:with-param name="pSeq" select="$root/groupsets/group[@defined = '1']/@begin"></xsl:with-param></xsl:call-template></xsl:variable>
    ...
    <xsl:variable name="reportTitle">
        <xsl:choose xml:space="preserve">
            ...
            <xsl:when test="$reportType = 'SymbolCCByGroup'">Highest Cyclomatic Complexity Coverage Top <xsl:value-of select="$ccClassCount" /> Per Group</xsl:when>
        ...
        <xsl:if test="$qualifiedmethods > 0">
            <tr>
                ...
                <xsl:if test="$reportCC">
                    <td>
                        <xsl:attribute xml:space="preserve" name="class"><xsl:if test="$showBottomLine = 'True'">mb </xsl:if>mR</xsl:attribute>
                        <xsl:if test="string($hideavgcc) != 'True'">
                            <xsl:value-of select="$stats/@ccavg" /> /
                        </xsl:if>
                        <xsl:value-of select="$stats/@ccmax" />
                    </td>
                </xsl:if>
                <xsl:if test="$reportVisitCounts">
    ...
    <xsl:template name="cyclomaticComplexitySummaryCommon">
        ...
        <xsl:param name="printMethods" xml:space="preserve"><xsl:value-of select="$displaymethods" /></xsl:param>
        ...
        <xsl:for-each select="$root/mod[string(stats/@ex) != '1']">
            ...
            <!--If you go outside of the loop you loose context (aka cc groups)-->
            <xsl:variable name="curMod" select="." />
            <!--
            ...
            <xsl:if test="$classcount > 0">
                ...
                <!--We break this down by groups so we can't show this header-->
                <xsl:if test="$reportType != 'SymbolCCByGroup'">
                    <tr>
                ...
                <xsl:choose>
                    ...
                    <xsl:when test="$reportType = 'SymbolCCByGroup'">
                        <!--
                        We need to determine for each group if there are any items in that group
                        We will also grab the top ccClassCount items for each group to keep the report
                        from getting out of hand.
                        -->
                        <xsl:for-each select="$root/groupsets/group[@defined = '1']">
                            <xsl:sort select="@end" data-type="number" order="descending" />
                            <xsl:variable name="begin" select="@begin" />
                            <xsl:variable name="end" select="@end" />
                            <!--We want greater or equal begin and less than end for grouping.-->
                            <xsl:variable name="groupNodes" select="$curMod/ns/class[string(stats/@ex) != '1' 
                                          and stats/@ccmax >= $begin and stats/@ccmax &lt; $end]" />
                            <xsl:if test="count($groupNodes) > 0">
                                <tr>
                                    <td class="secondaryChildTable ctHeader" colspan="{$tablecolumns}">
                                        Classes with a Cyclomatic Complexity
                                        <xsl:choose>
                                            <xsl:when test="position() = 1">
                                                greater than <xsl:value-of select="$begin" />
                                            </xsl:when>
                                            <xsl:otherwise>
                                                between  <xsl:value-of select="$begin" /> and <xsl:value-of select="$end" />
                                            </xsl:otherwise>
                                        </xsl:choose>
                                    </td>
                                </tr>
                                <xsl:for-each select="$groupNodes">
                                    <xsl:if test="position() &lt;= $ccClassCount">
                                        <xsl:call-template name="classCommon">
                                            <xsl:with-param name="printMethods" select="'False'" />
                                        </xsl:call-template>
                                            <!--We need to custom select the methods if we are going to display them-->
                                        <xsl:if test="$printMethods = 'True'">
                                            <xsl:variable name="methodsToPrint" select="method[string(stats/@ex) != '1' 
                                                and stats/@ccmax >= $ccMethodDefinedMin and stats/@ccmax &lt; $end]" />
                                            <xsl:for-each select="$methodsToPrint">
                                                <xsl:sort select="stats/@ccmax" data-type="number" order="descending"  />
        ...
        <xsl:choose>
            <xsl:when test="$isDiffReport">
                <xsl:call-template name="diffCoverageDetail">
                    ...
                    <xsl:with-param name="showBottomLine">True</xsl:with-param>
                    <xsl:with-param name="hideavgcc" select="'True'" />
                </xsl:call-template>
            </xsl:when>
            <xsl:when test="$isTrendReport">
                <xsl:call-template name="trendsCoverageDetail">
    ...
    <xsl:template name="qualifiednodes" xml:space="preserve"><xsl:choose>
        ...
        <xsl:otherwise><xsl:choose>
           <xsl:when test="stats[@usp] + stats[@vsp] = 0">1</xsl:when>
